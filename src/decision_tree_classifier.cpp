...
std::unordered_map<int, double> DecisionTreeClassifier::calculate_probabilities(
    const std::vector<DataPoint<int>>& data, const std::vector<size_t>& indices) {
...
int DecisionTreeClassifier::get_majority_class_in_node(const std::vector<DataPoint<int>>& data, const std::vector<size_t>& indices) {
...
std::pair<Node<int>*, bool> DecisionTreeClassifier::find_parent(Node<int>* root, Node<int>* target) {
    if (root == nullptr || target == nullptr) return {nullptr, false};

    if (auto* internal = dynamic_cast<ClassificationInternalNode*>(root)) {
...
int DecisionTreeClassifier::count_subtree_leaves(Node<int>* node){
    if (node == nullptr) {
        return 0;
    }
    ...
    if (const auto internal_node = dynamic_cast<ClassificationInternalNode*>(node)) {
        return count_subtree_leaves(internal_node->get_left_child()) + count_subtree_leaves(internal_node->get_right_child());
    }
    return 0;
}

void DecisionTreeClassifier::prune_node_to_leaf(const ClassificationInternalNode* node_to_prune,
        Node<int>* parent,
        bool is_left_child) {
    ...
    if (parent != nullptr) {
        auto* parent_internal = dynamic_cast<ClassificationInternalNode*>(parent);
...
bool DecisionTreeClassifier::is_leaf_node(Node<int>* node) {
...
std::pair<double, int> DecisionTreeClassifier::calculate_tree_error(Node<int>* node) {
    if (dynamic_cast<ClassificationLeafNode*>(node) != nullptr) {
...
void DecisionTreeClassifier::cost_complexity_prune() {
    ...
    while (!is_leaf_node(root.get()) && iteration < MAX_ITERATIONS) {
        ...
        prune_node_to_leaf(dynamic_cast<ClassificationInternalNode*>(weakest_node), parent, is_left);
    }
}
